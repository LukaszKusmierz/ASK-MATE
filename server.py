...
def registration():
    if request.method == 'GET':
        ...
        if len(password) not in config.PASSWORD_LENGTH:
            errors.append(f"Password should have from {config.PASSWORD_LENGTH_MIN} to"
                          f"{config.PASSWORD_LENGTH_MAX} characters.")
        if len(username) not in config.USERNAME_LENGTH:
            errors.append(f"Username should have from {config.USERNAME_LENGTH_MIN} to"
                          f"{config.USERNAME_LENGTH_MAX} characters.")
...
def login():
    if request.method == 'GET':
        ...
        if not user:
            return render_template("login.html", errors=[f'{username_email} not exist'])
...
def user_detail_page(user_id):
    user = util.user_data(user_id)
    user_page_data = [user]

    questions = util.user_questions_data(user_id)
    if questions:
        user_page_data.append(questions)

    answers = util.user_answers_data(user_id)
    if answers:
        user_page_data.append(answers)

    comments = util.user_comments_data(user_id)
    if comments:
        user_page_data.append(comments)

    return render_template('user_details_page.html', user_page_data=user_page_data)
...
def list_questions():
    ...
    columns = config.LIST_QUESTIONS_COLUMNS
...
def delete_comments(comment_id):
    user_id = session['user_id']
    question_id = data_manager.get_question_id_by_comment_id(comment_id) or \
        data_manager.get_question_id_by_answer_id(data_manager.get_answer_id_by_comment_id(comment_id))
